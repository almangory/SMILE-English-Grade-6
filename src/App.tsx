...
export default function App() {
  ...
  // PWA Install Prompt and Online/Offline Listeners
  useEffect(() => {
  ...
  const speakText = (text: string, voiceName: string = "Kore") => {
    ...
    // If offline or System Voice mode is selected, speak synchronously via native speech engine (100% offline guarantee)
    if (!navigator.onLine || voiceMode === "system") {
  ...
  return (
    <div className="min-h-screen bg-sky-50/50 flex flex-col font-sans select-none antialiased p-3 sm:p-6 pb-24 md:pb-6">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-wider bg-sky-100 text-sky-800 px-2 py-0.5 rounded-full border border-sky-200">Book 6</span>
              </div>
              <p className="text-sky-700 font-bold text-xs sm:text-sm mt-1">SMILE Grade 6 • Sudanese Curriculum</p>
            </div>
          </div>
              ...
              {activeTab === "book" && (
                ...
                >
                  ...
                  <div className="bg-white p-4 sm:p-8 rounded-[32px] sm:rounded-[40px] border-b-8 border-r-8 border-indigo-100 flex flex-col gap-4 relative overflow-hidden">
                    {/* 🎨 Dedicated, instant-loading 0ms illustration for EVERY single lesson */}
      ...
      <footer className="max-w-6xl w-full mx-auto mt-8 py-6 text-center text-sky-700/60 font-semibold border-t border-sky-100 mb-16 md:mb-0">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 text-xs uppercase tracking-wider font-extrabold px-4">
          <div className="flex items-center gap-2">
            <img src="/favicon.png" alt="NAQLA" className="w-5 h-5 rounded-md" />
            <p>© NAQLA English App • SMILE Grade 6 Sudan</p>
          </div>
          <div className="flex gap-3">
            <span className="text-sky-800">Pupil's Book 6</span>
            <span>•</span>
            <span className="text-emerald-700">100% Offline Ready</span>
          </div>
        </div>
      </footer>

      {/* 📱 Mobile Parallel Design: Bottom Navigation Bar (Fixed for Mobile Devices) */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-lg border-t-2 border-slate-200/80 px-2 py-2 flex justify-around items-center shadow-2xl">
        {[
          { id: "book", label: "الدروس", icon: BookOpen, color: "text-sky-600", activeBg: "bg-sky-50 text-sky-700 border-sky-300" },
          { id: "dictionary", label: "الكلمات", icon: Compass, color: "text-emerald-600", activeBg: "bg-emerald-50 text-emerald-700 border-emerald-300" },
          { id: "quiz", label: "الاختبار", icon: Award, color: "text-amber-600", activeBg: "bg-amber-50 text-amber-700 border-amber-300" },
          { id: "adventure", label: "المحادثة", icon: Smile, color: "text-purple-600", activeBg: "bg-purple-50 text-purple-700 border-purple-300" },
          { id: "syllabus", label: "الخطة", icon: Grid, color: "text-rose-600", activeBg: "bg-rose-50 text-rose-700 border-rose-300" },
          { id: "print", label: "الامتحانات", icon: Printer, color: "text-indigo-600", activeBg: "bg-indigo-50 text-indigo-700 border-indigo-300" }
        ].map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-1 px-2 rounded-xl transition-all cursor-pointer ${
                isActive 
                  ? `${item.activeBg} font-black scale-105 border shadow-sm` 
      ...
      <AnimatePresence>
        {showInstallModal && (
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" style={{ direction: "rtl" }}>
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="bg-white rounded-[32px] p-6 sm:p-8 max-w-md w-full shadow-2xl border-4 border-sky-400 relative"
            >
              <button
              ...
              <div className="flex items-center gap-3 mb-4">
                <img src="/favicon.png" alt="NAQLA App Icon" className="w-14 h-14 rounded-2xl shadow-md border" />
                <div>
                  <h3 className="text-xl font-black text-slate-800 leading-tight">تثبيت تطبيق NAQLA English</h3>
                  <p className="text-xs text-slate-500 font-bold">يعمل بدون إنترنت 100% كتطبيق هاتف</p>
                </div>
              </div>

              <div className="bg-sky-50 p-4 rounded-2xl border border-sky-100 mb-4 flex flex-col gap-3 text-sm text-slate-700 font-medium">
                <div className="flex items-start gap-2.5">
                  <span className="text-lg">📱</span>
                  <div>
                    <strong className="block text-sky-950 font-bold">على هواتف أندرويد وكروم (Android / Chrome):</strong>
                    <span>اضغط على خيارات المتصفح (الثلاث نقاط ⋮) في أعلى الشاشة، ثم اختر <span className="text-sky-700 font-bold">"تثبيت التطبيق (Install App)"</span> أو "إضافة إلى الشاشة الرئيسية".</span>
                  </div>
                </div>

                <div className="flex items-start gap-2.5 border-t border-sky-100 pt-2.5">
                  <span className="text-lg">🍏</span>
                  <div>
                    <strong className="block text-sky-950 font-bold">على أجهزة آيفون وآيباد (iPhone / Safari):</strong>
                    <span>اضغط على زر المشاركة السفلي (Share <span className="font-mono">⎋</span>)، ثم مرر لأسفل واختر <span className="text-sky-700 font-bold">"إضافة إلى الشاشة الرئيسية (Add to Home Screen ⊞)"</span>.</span>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 border-t border-sky-100 pt-2.5">
                  <span className="text-lg">💻</span>
                  <div>
                    <strong className="block text-sky-950 font-bold">على الكمبيوتر (Chrome / Edge):</strong>
                    <span>اضغط على أيقونة التثبيت <span className="font-mono">⊕</span> الموجودة في شريط العنوان أعلى المتصفح.</span>
                  </div>
                </div>
              </div>
              ...
              <button
                onClick={() => setShowInstallModal(false)}
                className="w-full bg-sky-600 hover:bg-sky-700 text-white font-black py-3.5 px-6 rounded-2xl shadow-md border-b-4 border-sky-800 transition-all active:scale-95"
              >
                فهمت ذلك، شكراً لك! 👍
              </button>
            </motion.div>
